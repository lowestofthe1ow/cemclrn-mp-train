...
def createTables(cursor):
  cursor.execute("CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) UNIQUE NOT NULL)")   
  cursor.execute("CREATE TABLE IF NOT EXISTS signatures (id INT AUTO_INCREMENT PRIMARY KEY, user_id INT NOT NULL, path VARCHAR(255) NOT NULL, time_added TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id))")
  db.commit()
...
def add_signature(user_id, path):
  cursor.execute("INSERT INTO signatures (user_id, path) VALUES (%s, %s)", (user_id, path))
  db.commit()

# Get 30 most recent signatures
def get_30_most_recent(user_id):
  cursor.execute("SELECT path FROM signatures WHERE user_id = %s ORDER BY time_added DESC LIMIT 30", (user_id,))
...
async def register(
    name: str,
    files: list[fa.UploadFile] = fa.File(...)
):
...
async def inference(
    name: str,
    new_signature: fa.UploadFile = fa.File(...)
):
...
async def update(
  ...
  if len(recent) > 30:
    return {"Status": "Registered", "Signature Count": 67} # placeholder, fine-tune model based on most recent 30 samples
